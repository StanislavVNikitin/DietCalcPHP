...
function findFoodToDiet($foodid)
{
    ...
    if (isset($_SESSION['auth']['login'])) {
        ...
        $sql = "SELECT * FROM diets 
                    WHERE foods_id = '{$foodid}' AND user_id='{$userid}'";
    } else {
        $guestsessionid = getGuestSessionId();
        $sql = "SELECT * FROM diet_temp 
                    WHERE foods_id = '{$foodid}' AND guestsessionid = '{$guestsessionid}'";
    }
...
function getDiet()
{
    if (isset($_SESSION['auth']['login'])) {
        ...
        $sql = "SELECT  d.id,
                ...
                WHERE d.foods_id = f.id AND d.user_id='{$userid}'";
    } else {
        $guestsessionid = getGuestSessionId();
        $sql = "SELECT  d.id,
                        f.name,
    ...
    if (!empty($_POST)) {
        if (isset($_SESSION['auth']['login'])) {
            ...
            if (!findFoodToDiet($foodid)) {
                $sql = "INSERT INTO diets (user_id, foods_id, count) 
                        VALUES ('{$userid}', '{$foodid}', '{$foodcount}')";
                $message = 'ok';
            } else {
                $sql = "UPDATE diets SET `count` = `count` +'{$foodcount}', `updated_at` = NOW() 
                        WHERE `diets`.`foods_id` = '{$foodid}' AND user_id = '{$userid}'";
                $message = 'edit';
            }
        } else {
            $guestsessionid = getGuestSessionId();
            ...
            $foodcount = (int)($_POST['foodcount']);
            if (!findFoodToDiet($foodid)) {
                $sql = "INSERT INTO diet_temp (guestsessionid, foods_id, count) 
                        VALUES ('{$guestsessionid}', '{$foodid}', '{$foodcount}')";
                $message = 'ok';
            } else {
                $sql = "UPDATE diet_temp SET `count` = `count` +'{$foodcount}', `updated_at` = NOW() 
                        WHERE `diet_temp`.`foods_id` = '{$foodid}' AND guestsessionid = '{$guestsessionid}'";
                $message = 'edit';
            }
        }
    }
...
function deleteFoodDiet($id)
{
    if (isset($_SESSION['auth']['login'])) {
        if (isset($id)) {
            $userid = $_SESSION['auth']['id'];
            $iddel = (int)$id;
            $sql = "DELETE FROM diets 
                    WHERE id = '{$iddel}' AND user_id = '{$userid}'";
            executeSql($sql);
        }
    } else {
        if (isset($id)) {
            $guestsessionid = getGuestSessionId();
            $iddel = (int)$id;
            $sql = "DELETE FROM diet_temp 
                    WHERE id = '{$iddel}' AND guestsessionid = '{$guestsessionid}'";
            executeSql($sql);
        }
    }
}
...
function editFoodInDiet(&$params, $id)
{
    ...
    if (isset($_SESSION['auth']['login'])) {
        ...
        $sql = "SELECT d.id,
                ...
                AND d.id = '{$idfood}' AND d.user_id='{$userid}'";
    } else {
        $guestsessionid = getGuestSessionId();
        $sql = "SELECT d.id,
                d.foods_id,
                ...
                FROM `diet_temp` as d, `foods` as f 
                WHERE d.foods_id = f.id 
                AND d.id = '{$idfood}' AND d.guestsessionid = '{$guestsessionid}'";
    }
...
function saveDiet()
{
    if (!empty($_POST)) {
        ...
        if (isset($_SESSION['auth']['login'])) {
            $userid = $_SESSION['auth']['id'];
            $sql = "UPDATE diets SET `count` = '{$foodcount}', `updated_at` = NOW() 
                        WHERE `diets`.`id` = '{$idfoodtodiet}' AND user_id='{$userid}'";

        } else {
            $guestsessionid = getGuestSessionId();
            $sql = "UPDATE diet_temp SET `count` = '{$foodcount}', `updated_at` = NOW() 
                        WHERE `diet_temp`.`id` = '{$idfoodtodiet}' AND guestsessionid = '{$guestsessionid}'";
        }
    }
    echo executeSql($sql);
}